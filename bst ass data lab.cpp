...
*/
    ...
    int min_diff(node *root_iter,int diff=0){
        	diff=minDiff(root_iter);
        	return diff;
        }
void inorder(node* curr, node* prev, int& diff)  
{  
    if (curr == NULL) {
        return;  
	}
    inorder(curr->left, prev, diff);  
    
    if (prev != NULL) {
        diff = min(curr->data - prev->data,diff);
    prev = curr; 
}
    inorder(curr->right, prev,diff);  
}  
...
int main() {
   ...
   */
   ...
   // int x=bst.count_nodes(bst.Get_Root(),29,0);
   // cout<<"number of nodes are "<<x<<endl;
   // int y=bst.right_leaves(bst.Get_Root());
   // cout<<"right leaf  is "<<y;
